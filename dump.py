...
if __name__ == '__main__':
    ...
    parser.add_argument('-H', '--host', dest='ssh_host', help='Specify SSH/remote hostname')
    ...
    parser.add_argument('-R', '--remote', dest='remote', action='store_true', help='Access device remotely (requires SSH hostname specified)')
    ...
    if not len(sys.argv[1:]) or (args.remote and not args.ssh_host):
        parser.print_help()
        sys.exit(exit_code)

    device = get_remote_iphone(args.ssh_host) if args.remote else get_usb_iphone()
